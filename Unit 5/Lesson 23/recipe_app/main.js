...
router.use(
  expressSession({
    ...
    cookie: {
      maxAge: 4_000_000, // this is in miliseconds, this is about an hour
    },
    resave: false, // we keep it false no changes to the session, then we dont want to save it. eg, adding things to a cart or settings prefrences
...
// we are adding this before the /show is because the code runs from top to bottom, so if someone is not logged in and it tries to run /users/:id it will crash. It will try to show a user with the id of "login". 
router.get("/users/login", usersController.login); // this is a get request, because it gets the page for us
router.post(
  "/users/login", // this is a post request because we are filling in a form, hitting submit and it is posting this action. 
